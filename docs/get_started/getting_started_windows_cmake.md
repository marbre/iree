...
`PATH` and <DO NOT SUBMIT windows sdk path/etc>; the easiest way to get this
configured is to use the `vsvarsall.bat` or `vcvars64.bat` files to set your
environment. See
...
> ..\iree-build\iree\tools\iree-translate.exe \
  ...
  -iree-llvm-target-cpu=znver2 \
  -iree-llvm-target-cpu-features=+sse2,+xsaves,... \
  $PWD/iree/tools/test/simple.mlir \
  -o /tmp/simple-llvm_aot.vmfb
```
